...
create-venv:
	...
	@printf "[Makefile] - Creating a virtual environment named 'auth-api-3.9'...\n"
	@if [ ! -d "auth-api-3.9" ]; then \
		python3.9 -m venv auth-api-3.9; \
	else \
		printf "[Makefile] - Virtual environment 'auth-api-3.9' already exists!\n"; \
	fi
	@printf "[Makefile] - Virtual environment setup complete. To activate, run:\n"
	@printf "[Makefile] - 'source auth-api-3.9/bin/activate'\n"

#* Activate the virtual environment
.PHONY: activate-venv
activate-venv:
	@printf "[Makefile] - Activating the virtual environment 'auth-api-3.9'...\n"
	@source auth-api-3.9/bin/activate && printf "[Makefile] - Virtual environment activated!\n"
...
.PHONY: start-airflow
start-airflow:
	@printf "[Makefile] - Setting up Airflow home to 'src/auth_api'.\n"
	@AIRFLOW_HOME=$(shell pwd)/src/auth_api \
...
start-jupyterlab:
	...
	@chmod +x src/auth_api/experiments/start_jupyterlab.sh
	@if [ ! -f src/auth_api/experiments/start_jupyterlab.sh ]; then \
		printf "[Makefile] - start_jupyterlab.sh not found!\n"; \
		exit 1; \
	fi
	@src/auth_api/experiments/start_jupyterlab.sh

# Set GH Workflows Variables
.PHONY: setup-env
setup-env:
	@printf "[Makefile] - Setting up environment variables...\n"
	@LOCAL_DAGS_PATH="src/auth_api/dags"; \
	COMPOSER_BUCKET="gs://auth_api"; \
	DOCKER_IMAGE_TAG="$(IMAGE):$(VERSION)-$${SANITIZED_TAG}"; \
	DOCKER_FILE_TAG="docker_version.txt"; \
	ARTIFACT_REGISTRY_URI="us-central1-docker.pkg.dev/auth_api-dev"; \
...
sync-files:
	...
	# Sync auth_api folder
	gsutil -m rsync -x 'endpoints/.*|experiments/.*|dags/.*dag\.py' -r src/auth_api $(COMPOSER_BUCKET)/dags/auth_api
